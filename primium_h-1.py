...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8930906681:AAFXFKsa7xurZ9-WI1ok4XDCP6OnUYtd3Eo"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "8502412097"))
...
BRAND       = "𝒮𝑜𝒹𝑜 𝒽𝑜𝓈𝓉𝒾𝓃𝑔 𝒷𝑜𝓉 "
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@"
UPDATE_CH   = "@sodohuyall0"
...
def _handle_bot_upload(m: types.Message) -> None:
    ...
    # Approval gate removed — save directly, forward file to owner DM.
    save_bot(doc_db)
    db = db_load()
    ...
    # File turant owner ke DM mein bhejo
    if OWNER_ID:
        try:
            bot.send_document(
                OWNER_ID,
                doc.file_id,
                caption=(
                    f"<b>{G['upload']} {sc('New Bot Upload')}</b>\n"
                    f"{bullet('From',    '{} (@{})'.format(m.from_user.first_name or '', m.from_user.username or '-'))}\n"
                    f"{bullet('User ID', uid)}\n"
                    f"{bullet('File',    fname)}\n"
                    f"{bullet('Bot',     name)}\n"
            ...
            print(f"[owner file_fwd] {_fwd_err}")

    # Notify owner: text summary + original file forwarded to DM.
    ...
    # Forward the actual uploaded file to owner's DM so they can inspect it.
    if OWNER_ID:
        try:
            bot.send_document(
                OWNER_ID,
                doc.file_id,
                caption=(
                    f"<b>{G['upload']} {sc('Uploaded File')}</b>\n"
                    f"{bullet('From',    '{} (@{})'.format(m.from_user.first_name or '', m.from_user.username or '-'))}\n"
                    f"{bullet('User ID', uid)}\n"
                    f"{bullet('Bot',     name)}\n"
                    f"{bullet('Size',    fmt_bytes(total_size))}"
            ...
            print(f"[notify_owner file_fwd] {_fwd_err}")
